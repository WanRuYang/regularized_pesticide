...
--
--
--DROP TABLE IF EXISTS  lynn.udc_fld_grp_uc;
--CREATE TABLE lynn.udc_fld_grp_uc(
...
--);
--
--INSERT INTO lynn.udc_fld_grp_uc(
...
--	INNER JOIN lynn.fields_uc s ON trim(u.grower_id)||'_'||trim(u.site_loc_id)||'_'||(u.site_code::text) =  s.field_id
--   
...
DROP TABLE IF EXISTS lynn.comtrs_fld_sum_uc;
WITH t AS (
...
SELECT year, comtrs, county, ai_cd, dpr_nm AS chem, 
	...
	INTO lynn.comtrs_fld_sum_uc
	FROM t
	GROUP BY year, comtrs, county, ai_cd, dpr_nm;

UPDATE lynn.comtrs_fld_sum_uc SET km2_planted =5.2 WHERE km2_planted > 5.2;
UPDATE lynn.comtrs_fld_sum_uc SET km2_treated = km2_planted WHERE km2_treated > km2_planted;
...
SELECT year, comtrs, chem_code AS ai_cd, dpr_nm AS chem, county_cd AS county, 
		SUM(lbs_chm_used)*0.45359237 AS kg, 
		SUM(acre_planted)*0.00404685642 AS km2_planted, 
		SUM(acre_treated)*0.00404685642 AS km2_treated
	INTO lynn.un_counted_comtrs
	FROM lynn.un_counted_fld f
	INNER JOIN lynn.croplist c  ON c.ai_cd = f.chem_code
	GROUP BY year, comtrs, chem_code, dpr_nm, county_cd;

UPDATE lynn.un_counted_comtrs SET km2_planted =5.2 WHERE km2_planted > 5.2;
...
--CREATE TABLE lynn.year_chem_trim_uc(year int, chem text, kg numeric);

WITH a AS (SELECT year, ai_cd, chem, u.comtrs, county_cd AS county, --substring(comtrs, 1, 2) AS county, 
					kg, km2_treated, kg/km2_treated AS unit_rate
	             FROM lynn.comtrs_fld_sum_uc u
		  	     INNER JOIN lynn.comtrs_cntlist t ON t.comtrs = u.comtrs
		  UNION
		  SELECT year, ai_cd, chem, u.comtrs, county, kg, km2_treated, kg/km2_treated AS unit_rate
                 FROM lynn.un_counted_comtrs u),
...
DROP TABLE IF EXISTS lynn.year_chem_trim_uc_final;
 WITH a AS (
     SELECT year, chem, sum(kg) AS kg
	 FROM lynn.comtrs_to_add_uc
	 GROUP BY year, chem)
 ...
 SELECT t.year, t.chem, t.ai_cd,
 	...
 	INTO lynn.year_chem_trim_uc_final
 	FROM lynn.year_chem_trim_uc t
